...
async def update_campaign(campaign_id: UUID, data: CampaignUpdate, client: Client = Depends(get_current_active_client), db: AsyncSession = Depends(get_db)):
    ...
    campaign = result.scalar_one()
    # Fetch locations separately to avoid stale cache
    from app.models.location_profile import LocationProfile as FreshLP
    lp_result = await db.execute(select(FreshLP).where(FreshLP.campaign_id == campaign.campaign_id))
