...
async def update_campaign(campaign_id: UUID, data: CampaignUpdate, client: Client = Depends(get_current_active_client), db: AsyncSession = Depends(get_db)):
    ...
    # Handle location updates (replace all)
    if hasattr(data, 'locations') and data.locations is not None:
        from app.models.location_profile import LocationProfile
        from app.services.geocoding_service import get_geocoding_service
        from app.services.elevation_service import get_pressure_range
        from app.services.magnetic_field_service import get_magnetic_field_range
        ...
        sub_result = await db.execute(select(Subscription).where(Subscription.client_id == client.client_id))
        sub = sub_result.scalar_one_or_none()
        tier_str = sub.tier.value if sub and sub.tier and hasattr(sub.tier, 'value') else "free"
        tier_limits = {"free": 5, "pro": 500, "enterprise": 99999}
        max_locs = tier_limits.get(tier_str.lower(), 5)
        if len(data.locations) > max_locs:
            raise HTTPException(status_code=status.HTTP_402_PAYMENT_REQUIRED, detail=f"Your {tier_str} plan allows {max_locs} locations per campaign.")
        ...
        for loc_data in data.locations:
            ...
            resolved = None
            tol = loc_data.get('tolerance_meters', 100)
            has_c = loc_lat is not None and loc_lon is not None
            has_a = loc_addr and str(loc_addr).strip()
            if has_a and not has_c:
                try:
                    gr = await geocoding_svc.geocode_address(str(loc_addr))
                    if gr: loc_lat, loc_lon, resolved, has_c = gr.latitude, gr.longitude, gr.formatted_address, True
                except Exception: pass
            elif has_c and not has_a:
                try:
                    gr = await geocoding_svc.reverse_geocode(float(loc_lat), float(loc_lon))
                    if gr: resolved = gr.formatted_address
                except Exception: pass
            elif has_a and has_c:
                resolved = str(loc_addr)
            if not has_c: continue
            p_min = p_max = m_min = m_max = None
            try:
                pr = await get_pressure_range(float(loc_lat), float(loc_lon))
                if pr: p_min, p_max = pr
            except Exception: pass
            try:
                mr = await get_magnetic_field_range(float(loc_lat), float(loc_lon))
                if mr: m_min, m_max = mr
            except Exception: pass
            db.add(LocationProfile(campaign_id=campaign.campaign_id, expected_latitude=float(loc_lat), expected_longitude=float(loc_lon), tolerance_meters=float(tol), expected_pressure_min=p_min, expected_pressure_max=p_max, expected_magnetic_min=m_min, expected_magnetic_max=m_max, resolved_address=resolved))
