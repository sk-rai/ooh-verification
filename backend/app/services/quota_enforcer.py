...
class QuotaEnforcer:
    ...
    async def check_photo_quota(self, client_id: str) -> bool:
        ...
        if subscription.status != SubscriptionStatus.ACTIVE:
            raise QuotaExceededError(
                ...
                quota_type="subscription_status", current=0, limit=0,
        ...
        if subscription.photos_used >= subscription.photos_quota:
            raise QuotaExceededError(
                ...
                quota_type="photos", current=subscription.photos_used,
                limit=subscription.photos_quota, tier=subscription.tier.value
            )
        return True
    ...
    async def check_vendor_quota(self, client_id: str) -> bool:
        ...
        if subscription.vendors_used >= subscription.vendors_quota:
            raise QuotaExceededError(
                ...
                quota_type="vendors", current=subscription.vendors_used,
                limit=subscription.vendors_quota, tier=subscription.tier.value
            )
        return True
    ...
    async def check_campaign_quota(self, client_id: str) -> bool:
        ...
        if subscription.campaigns_used >= subscription.campaigns_quota:
            raise QuotaExceededError(
                ...
                quota_type="campaigns", current=subscription.campaigns_used,
                limit=subscription.campaigns_quota, tier=subscription.tier.value
            )
        return True
    ...
    async def check_storage_quota(self, client_id: str, file_size_mb: float) -> bool:
        ...
        if new_usage > subscription.storage_quota_mb:
            raise QuotaExceededError(
                ...
                quota_type="storage", current=int(subscription.storage_used_mb),
                limit=subscription.storage_quota_mb, tier=subscription.tier.value
            )
        return True
    ...
    async def sync_usage_counters(self, client_id: str) -> None:
        ...
        # FIX 3: Vendor.created_by_client_id instead of Vendor.client_id
