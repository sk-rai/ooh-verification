...
class EnvironmentalData(BaseModel):
    ...
    def normalize(self):
        ...
        # If Android detected tremor (motion present), treat as human — the classification
        # can be too strict. A real person holding a phone WILL have tremor.
        if self.tremor_detected is True:
            self.hand_tremor_is_human = True
            ...
            if isinstance(cell_data, dict) and 'towers' in cell_data:
                ...
                for t in cell_data['towers']:
                    towers.append(CellTower(
                        cell_id=t.get('cell_id', 0), lac=t.get('lac', 0),
                        mcc=t.get('mcc', 0), mnc=t.get('mnc', 0),
...
class PhotoSignatureSchema(BaseModel):
    ...
    def validate_algorithm(cls, v):
        ...
        if v not in valid_algorithms:
            sep = ', '
            raise ValueError(f"Invalid algorithm. Must be one of: {sep.join(valid_algorithms)}")
        return v
...
class PhotoUploadResponse(BaseModel):
    ...
    distance_from_expected: Optional[float] = Field(None, description="Distance from expected in meters")
    verification_confidence: Optional[float] = Field(None, description="Overall confidence score (0-1)")
    verification_flags: Optional[List[str]] = Field(None, description="Verification flags")
    s3_url: str
